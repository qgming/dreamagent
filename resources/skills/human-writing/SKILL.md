...
5. `references/revision.md` 的改稿规则与 `check_prose` 工具的提醒。

具体文体覆盖通用写法。现实稿的来源要求不能压到纯虚构上，散文的完整主干不能压到诗歌分行和人物对白上。`check_prose` 只能执行已经写明的硬禁令，警告项不能替作者决定文体。
...
- 不用商业汇报和模型惯用黑话替普通事情抬价。`references/revision.md` 区分绝对禁词和需要结合语境判断的词，`check_prose` 不得自行增加未写明的硬禁词。
...
长稿初稿完成后调用内置工具 `check_prose({ content|path })` 复核，直到禁用项清零。
