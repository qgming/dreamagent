...
`check_prose` 工具里的硬禁词必须与本节完全一致。增加或删除硬禁词时，同一轮修改工具，不能让工具偷偷多出一套规则。
...
长稿初稿完成后调用内置工具 `check_prose({ content|path })` 复核。

工具会把冒号、破折号、翻案句、硬停词、模型路标和绝对禁用的黑话判为失败（failures）。它会把需要结合语境判断的词列为提醒（warnings），还会提醒短句排队、段落过度单一、固定开场与多套比喻。

工具只负责发现形状，不能判断文章有没有人。警告项需要回到材料与说话位置人工判断。诗歌、剧本和实验写作只把失败项当成硬要求，散文形状警告服从具体文体。
